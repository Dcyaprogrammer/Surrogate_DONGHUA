...
for epoch in range(num_epochs):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), 'SS_best_model.pth')

    print(f"Epoch {epoch+1}/{num_epochs}")
    print(f"Trian_loss: {train_loss:.4f} | Val_loss: {val_loss:.4f} | Val_Accuracy {val_acc:.4f}")


model.load_state_dict(torch.load('SS_best_model.pth'))
model.eval()
...
plt.tight_layout()
plt.savefig("SS_confusion_matrix.png", dpi=300)
plt.show()
...
plt.legend()
plt.grid(True)
plt.savefig('SS_loss_curves.png')
plt.show()
...
plt.legend()
plt.grid(True)
plt.savefig('SS_val_accuracy.png')
plt.show()
...
plt.tight_layout()  
plt.savefig('SS_loss_and_accuracy.png', dpi=300, bbox_inches='tight')
plt.show()


np.savetxt('SS_train_losses.txt', np.array(train_losses), fmt='%.6f')
np.savetxt('SS_val_losses.txt', np.array(val_losses), fmt='%.6f')
np.savetxt('SS_val_accuracies.txt', np.array(val_accuracies), fmt='%.6f')