...
''''''
def sigmoid_simulation(x):
    return 0.00118 + 0.99489 / (3.7941 * math.sqrt(math.pi / (4 * math.log(2)))) * math.exp(-4 * math.log(2) * (x-0.03421)^2 / 3.7941^2)
'''
双层（好曲线）
y=0.00118+ 0.99489/(3.7941*sqrt(pi/(4*ln(2)))) * exp(-4*ln(2)*(x-0.03421)^2/3.7941^2)
'''
...
class sigmoid_simulation_function(torch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, grad_output):
        return sigmoid_simulation_backward(grad_output, ctx.saved_tensors[0], ctx.alpha)

''''''
class Sigmoid_Simulation(SurrogateFunctionBase):
    def __init__(self, alpha=4.0, spiking=True):
        super().__init__(alpha, spiking)
...
def bbl(x):
    return 0.28 * (1 / (1 + math.exp(-(x + 0.35876 + 4.57243 / 2) / 1.12176))) * (
                1 - 1 / (1 + math.exp(-(x + 0.35876 - 4.57243 / 2) / 0.5924)))


'''
bbl
y = 0.28 * (1 / (1 + exp(-(x + 0.35876 + 4.57243 / 2) / 1.12176))) * (1 - 1 / (1 + exp(-(x + 0.35876 - 4.57243 / 2) / 0.5924)))
'''
...
class bbl_function(torch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, grad_output):
        return bbl_backward(grad_output, ctx.saved_tensors[0], ctx.alpha)


''''''


class bbl_Simulation(SurrogateFunctionBase):
    def __init__(self, alpha=4.0, spiking=True):
        super().__init__(alpha, spiking)
